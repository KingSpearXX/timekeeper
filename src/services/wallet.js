...
export const walletService = {
  ...
  async getUserWalletLogs(userId, page = 1, limit = 10) {
    try {
      ...
      if (!response.ok) {
        ...
        console.error('Wallet logs fetch failed:', response.status, response.statusText)
  ...
  async getUserWalletBalance(userId) {
    try {
      ...
      if (!response.ok) {
        const errorText = await response.text()
        console.error('Wallet balance fetch failed:', response.status, response.statusText)
  ...
  async createWithdrawal(userId, amount, reason = '') {
    try {
      const token = getAuthToken()
      ...
      if (!response.ok) {
        const errorText = await response.text()
        console.error('Withdrawal creation failed:', response.status, response.statusText)
